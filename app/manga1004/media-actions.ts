...
export async function uploadCoverAction(titleId: string, formData: FormData) {
  ...
  try {
    ...
    const [uploaded] = await uploadImages(target.format, coverObjectPrefix(target.format, target.originalLanguage, target.slug, target.createdAt), [image], { singleFileName: "cover" });
