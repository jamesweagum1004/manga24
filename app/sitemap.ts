...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  if (!settings.sitemapEnabled) return entries;
  ...
  for (const locale of settings.enabledLocales) {
    ...
    if (settings.sitemapIncludeStatic) {
      entries.push(
    ...
    if (settings.sitemapIncludeTags) {
      ...
      for (const tagSlug of tagSlugs) entries.push(sitemapEntry(`/${locale}/tags/${tagSlug}`, "weekly", 0.6));
    }

    for (const title of localeTitles) {
      if (settings.sitemapIncludeTitles) {
        entries.push({ ...sitemapEntry(`/${locale}/manga/${title.slug}`, "daily", 0.8), lastModified: new Date(title.publishedAt) });
      }

      if (settings.sitemapIncludeChapters) {
        for (const chapter of title.chapters) {
          entries.push({ ...sitemapEntry(`/${locale}/manga/${title.slug}/chapter/${chapter.slug}`, "monthly", 0.7), lastModified: new Date(chapter.publishedAt) });
        }
      }
    }
  }
